...
planet_dic = {'Меркурий' : 'Mercury', 'Венера' : 'Venus', 'Марс' : 'Mars', 'Юпитер' : 'Jupiter', 'Сатурн': 'Saturn', 'Уран' : 'Uranus', 'Нептун' : 'Neptune', 'Плутон' : 'Pluto'}
...
def planet(bot,update):
    ...
    for plan in user_list:
        if plan in planet_dic:
            plan2 = planet_dic[plan]
            planet_object = getattr(ephem, plan2)
            res = planet_object(today)
            ephem.constellation(res)
            update.message.reply_text(ephem.constellation(res))
            update.message.reply_text(plan + ' сегодня в созвездии '+ const_dic[ephem.constellation(res)[0]])
